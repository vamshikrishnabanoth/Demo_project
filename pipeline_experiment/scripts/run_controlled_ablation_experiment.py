"""
Controlled 4-Way Architectural Ablation Benchmark:
A: Summary-Only (Baseline WHAT) -> MCQs
B: Blueprint-Only (Baseline WHY) -> MCQs
C: Unified (WHAT + WHY) -> Planner -> RAG -> MCQs -> Validator
E: Adaptive Production Pipeline (Router -> Representation -> Planner -> RAG -> MCQs -> Validator)

Evaluates:
- Content Grounding
- Teacher Intent & Salience
- Question Diversity & Redundancy
- Cognitive Distribution (Bloom's Entropy)
- Distractor Quality
- Evidence Traceability
- Measured LLM Calls & Latency
"""
...
def evaluate_suite_metrics(
    ...
    # Shannon Entropy for Cognitive Diversity (max 2.0 bits for 4 classes)
    entropy = 0.0
    ...
    # 5. Redundancy / Repetition Estimation (checking distinct question stems / targets)
    stems = [q.get("question_text", "").lower()[:40] for q in questions]
    ...
    # Weights: Grounding (25%), Cognitive Diversity (20%), Traceability (20%), Teacher Intent (20%), Non-redundancy (15%)
...
def run_pipeline_a(canonical: CanonicalEducationalInput, llm: UnifiedLLMEngine, count: int = 10) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        f"Task: Generate MCQs assessing this summary. Output JSON matching SimpleMCQListOutput."
    )
...
def run_pipeline_b(canonical: CanonicalEducationalInput, llm: UnifiedLLMEngine, count: int = 10) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        f"Task: Generate MCQs assessing these topics. Output JSON matching SimpleMCQListOutput."
    )
...
def run_pipeline_c_unified_planned(canonical: CanonicalEducationalInput, llm: UnifiedLLMEngine, count: int = 10) -> Dict[str, Any]:
    ...
    # Assessment Planner on Unified representation
    ...
    # RAG Retrieval
    evidence_map = ProductionEvidenceRetriever.retrieve_for_plan(canonical, plan)
...
def main():
    ...
    # 3. Pipeline C (Unified Full Pipeline)
    res_c = run_pipeline_c_unified_planned(canonical, llm, count=10)

    # 4. Pipeline E (Adaptive Production Engine)
    res_e = run_pipeline_e_production_adaptive(canonical, engine, count=10)

    results = {
        "benchmark": "Controlled 4-Way Architectural Ablation Study",
        "lecture": canonical.title,
        "audio_duration_minutes": 77.5,
        "pipeline_a": res_a,
        "pipeline_b": res_b,
        "pipeline_c": res_c,
        "pipeline_e": res_e
    ...
    out_file = "production_engine/outputs/controlled_ablation_results.json"
    os.makedirs("production_engine/outputs", exist_ok=True)
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print(f"\nSaved controlled ablation benchmark results to: {out_file}")
