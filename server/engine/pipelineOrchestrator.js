...
class PipelineOrchestrator {
  ...
  async runPipeline(sessionInputs = {}, progressCallback = null) {
    ...
    // ──────────────────────────────────────────────────────────────────────────
    const t2 = Date.now();
    const plan = await agent1Planner.planAssessment(evidencePackage, requestedDifficulty, requestedCount);
    ...
    for (let i = 0; i < primaryTargets.length; i++) {
      ...
      while (attempts < 3 && !targetPassed) {
        ...
        // 4C. Agent 3 Question-Level Reasoning Evaluation
